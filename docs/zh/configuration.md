...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `enabled` | bool | `true` | 是否启动 gRPC（需在 `infrastructure/run.go` 调用 `WithGRPC(registerGRPC)`） |
...
三层模型：**L1 全局 → L2 用户 → L3 路由/API**。框架 `app` 默认挂载 **L1** 与 **L3（config 配置了 `routes` 时）**；L2 在业务 `infrastructure/registerHTTP` 中挂载。

后端：`memory`（单进程/开发）或 `redis`（多 Pod 分布式，GCRA）。
...
相关环境变量见 [环境变量](#环境变量) 一节。

#### routes 规则说明（L3）

| 字段 | 说明 |
|------|------|
| `method` | HTTP 方法（`GET` / `POST` …）或 gRPC 固定写 `GRPC` |
| `path` | HTTP：Gin 路由模板（如 `/ai/chat`）；gRPC：FullMethod |
| `rps` / `burst` | 该路由独立令牌桶参数 |

配置 `routes` 后，`app.Run` 自动挂载 L3，无需在代码中重复注册。详见 [分层限流指南](rate-limiting.md)。
...
配置中 `api_key: "${OPENAI_API_KEY}"` 由 go-common 在加载时解析。加载完成后，`GOBLOCKS_*` 变量仍可覆盖对应字段（优先级更高）。
